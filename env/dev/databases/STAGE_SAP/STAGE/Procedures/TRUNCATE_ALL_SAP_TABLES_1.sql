...
--changeset CHANDRAMANI:TRUNCATE_ALL_SAP_TABLES_1 runOnChange:true failOnError:true endDelimiter:""
--comment TRUNCATE_ALL_SAP_TABLES procedure
-- IMPORTANT: this procedure is for development and test only and should not be promoted to PROD
CREATE OR REPLACE PROCEDURE "TRUNCATE_ALL_SAP_TABLES"()
RETURNS VARCHAR(16777216)
LANGUAGE SQL
EXECUTE AS OWNER
AS 
declare
    the_schema varchar default 'STAGE';
begin
    ...
    FOR actionable_table IN actionable_table_list DO    	
        ...
        execute immediate ('TRUNCATE TABLE ' || the_schema || '."' || table_name || '";');         
    END FOR;
    return 'Truncated all SAP tables in this schema';
end;